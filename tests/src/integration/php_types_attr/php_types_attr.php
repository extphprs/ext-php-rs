...
assert($type instanceof ReflectionUnionType, 'expected ReflectionUnionType, got ' . ( $type ? $type::class : 'null' ));
assert($params[0]->allowsNull() === false, 'must not be nullable');

$members = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $type->getTypes());
sort($members);
assert($members === ['int', 'string'], 'expected int|string, got ' . implode('|', $members));
...
assert($ret->allowsNull() === true, 'returns_int_string_or_null must allow null');

$members = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $ret->getTypes());
sort($members);
assert($members === ['int', 'null', 'string'], 'expected int|string|null on return, got ' . implode('|', $members));
...
assert($params[0]->allowsNull() === false, 'class union must not be nullable');
...
assert($ret->allowsNull() === false, 'class union return must not be nullable');
...
if (PHP_VERSION_ID >= 80_300) {
    ...
    assert(
        ...
        'intersection: expected ReflectionIntersectionType, got ' . ( $type ? $type::class : 'null' )
    );

    $members = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $type->getTypes());
    sort($members);
    assert($members === ['Countable', 'Traversable'], 'expected Countable&Traversable, got ' . implode('&', $members));
    ...
    foreach ($branches as $branch) {
        ...
        assert($branch instanceof ReflectionNamedType, 'dnf: unexpected branch class ' . $branch::class);
        $named[] = $branch->getName();
    }
    sort($named);
    assert($named === ['PhpTypesAttrFoo'], 'dnf: expected named branch PhpTypesAttrFoo, got ' . implode(',', $named));
    ...
    assert(
        ...
        'dnf: expected Countable&Traversable inner intersection, got ' . implode('&', $intersection_members)
    );
    ...
    assert(
        ...
        'intersection return: expected ReflectionIntersectionType, got ' . ( $ret ? $ret::class : 'null' )
    );
    ...
    foreach ($branches as $branch) {
        ...
        assert($branch instanceof ReflectionNamedType, 'dnf return: unexpected branch class ' . $branch::class);
        $named[] = $branch->getName();
    }
    sort($named);
    assert(
    ...
    assert(
        $intersection_members === ['Countable', 'Traversable'],
        'dnf return: expected Countable&Traversable inner intersection, got ' . implode('&', $intersection_members)
    );
}
...
assert(
    $type instanceof ReflectionUnionType,
    'PhpTypesAttrHolder::accept: expected ReflectionUnionType, got ' . ( $type ? $type::class : 'null' )
);
...
assert(
    $ret instanceof ReflectionUnionType,
    'PhpTypesAttrHolder::produce: expected ReflectionUnionType, got ' . ( $ret ? $ret::class : 'null' )
);
assert($ret->allowsNull() === true, 'PhpTypesAttrHolder::produce: must allow null on return');
...
assert(
    $ret instanceof ReflectionUnionType,
    'PhpTypesAttrHolder::produceClassUnion: expected ReflectionUnionType, got ' . ( $ret ? $ret::class : 'null' )
);
assert($ret->allowsNull() === false, 'PhpTypesAttrHolder::produceClassUnion: must not allow null on return');

$members = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $ret->getTypes());
sort($members);
assert(
    $members === ['PhpTypesAttrBar', 'PhpTypesAttrFoo'],
    'PhpTypesAttrHolder::produceClassUnion: expected PhpTypesAttrFoo|PhpTypesAttrBar, got ' . implode('|', $members)
);
...
if (PHP_VERSION_ID >= 80_300) {
    ...
    assert(
        $members === ['Countable', 'Traversable'],
        'PhpTypesAttrHolder83::produceIntersection: expected Countable&Traversable, got ' . implode('&', $members)
    );
    ...
    assert(
        $ret instanceof ReflectionUnionType,
        'PhpTypesAttrHolder83::produceDnf: expected ReflectionUnionType (DNF), got ' . ( $ret ? $ret::class : 'null' )
    );

    $branches = $ret->getTypes();
    assert(count($branches) === 2, 'PhpTypesAttrHolder83::produceDnf: expected two top-level branches');

    $named = [];
    $intersection = null;
    foreach ($branches as $branch) {
        if ($branch instanceof ReflectionIntersectionType) {
            assert($intersection === null, 'PhpTypesAttrHolder83::produceDnf: more than one intersection branch');
            $intersection = $branch;
            continue;
        }
        assert(
    ...
    assert(
        $named === ['PhpTypesAttrFoo'],
        'PhpTypesAttrHolder83::produceDnf: expected named branch PhpTypesAttrFoo, got ' . implode(',', $named)
    );

    assert($intersection !== null, 'PhpTypesAttrHolder83::produceDnf: missing intersection branch');
    $intersection_members = array_map(
        static fn(ReflectionNamedType $t): string => $t->getName(),
        $intersection->getTypes()
    );
