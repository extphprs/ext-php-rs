...
function dnf_member_shapes(ReflectionType $type): array
{
    ...
    foreach ($type->getTypes() as $member) {
        if ($member instanceof ReflectionIntersectionType) {
            $names = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $member->getTypes());
            sort($names);
...
assert($type instanceof ReflectionUnionType, 'test_dnf_arg: expected ReflectionUnionType');
assert($params[0]->allowsNull() === false, 'test_dnf_arg: must not allow null');
...
assert($type instanceof ReflectionUnionType, 'test_dnf_nullable_arg: expected ReflectionUnionType');
assert($rf->getParameters()[0]->allowsNull() === true, 'test_dnf_nullable_arg: must allow null');

// (DnfA&DnfB)|(DnfA&DnfD) arg.
$rf = new ReflectionFunction('test_dnf_two_intersections_arg');
$type = $rf->getParameters()[0]->getType();
assert($type instanceof ReflectionUnionType, 'test_dnf_two_intersections_arg: expected ReflectionUnionType');
$shapes = dnf_member_shapes($type);
assert(
    $shapes === [
    ...
    'test_dnf_two_intersections_arg: expected (DnfA&DnfB)|(DnfA&DnfD), got ' . json_encode($shapes)
);

// (DnfA&DnfB)|DnfC return.
$rf = new ReflectionFunction('test_dnf_returns');
$ret = $rf->getReturnType();
assert($ret instanceof ReflectionUnionType, 'test_dnf_returns: expected ReflectionUnionType return');
assert($ret->allowsNull() === false, 'test_dnf_returns: must not allow null');
...
$ret = $rf->getReturnType();
assert($ret instanceof ReflectionUnionType, 'test_dnf_nullable_returns: expected ReflectionUnionType return');
assert($ret->allowsNull() === true, 'test_dnf_nullable_returns: must allow null');
