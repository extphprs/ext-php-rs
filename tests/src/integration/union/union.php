...
foreach (['test_union_int_string_or_null', 'test_union_int_string_allow_null'] as $fname) {
    ...
    $members = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $type->getTypes());
    sort($members);
    assert($members === ['int', 'null', 'string'], "$fname: expected int|null|string, got " . implode('|', $members));
...
foreach ([
    ...
    assert($ret->allowsNull() === $case['nullable'], "{$case['fname']}: nullable mismatch");

    $members = array_map(static fn(ReflectionNamedType $t): string => $t->getName(), $ret->getTypes());
    sort($members);
    assert(
        $members === $case['members'],
        "{$case['fname']}: expected " . implode('|', $case['members']) . ', got ' . implode('|', $members)
    );
}
