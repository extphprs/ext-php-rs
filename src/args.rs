...
/// Represents a single element in a DNF type - either a simple class or an intersection group.
...
pub enum ArgType {
    ...
    /// A DNF (Disjunctive Normal Form) type (e.g., `(Countable&Traversable)|ArrayAccess`)
    /// Only available in PHP 8.2+.
    Dnf(Vec<TypeGroup>),
}
...
impl ArgType {
    ...
    /// For complex types, returns Mixed as a fallback for runtime type checking.
    #[must_use]
...
impl<'a> Arg<'a> {
    ...
    /// Creates a new argument with a DNF (Disjunctive Normal Form) type (PHP 8.2+).
    ///
    ...
    ///
    /// Note: Intersection, class union, and DNF types for internal function parameters
    /// are only supported in PHP 8.3+. On earlier versions, these fall back to `mixed` type.
...
impl From<Arg<'_>> for Parameter {
    fn from(val: Arg<'_>) -> Self {
        ...
        // TODO: Extend Parameter to support union/intersection/DNF types for better stub generation
