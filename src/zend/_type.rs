...
impl ZendType {
    ...
    pub fn allows_null(&self) -> bool {
        // Null is allowed if either the nullable bit is set OR if the type mask includes MAY_BE_NULL
        (self.type_mask & _ZEND_TYPE_NULLABLE_BIT) != 0
            || (self.type_mask & (1 << crate::ffi::IS_NULL)) != 0
    }

    /// Creates an intersection type from multiple class/interface names (PHP 8.1+).
    ///
    ...
    /// # Parameters
    ///
    /// * `class_names` - Slice of class/interface names that form the intersection.
    /// * `pass_by_ref` - Whether the value should be passed by reference.
    /// * `is_variadic` - Whether this type represents a variadic argument.
    ///
    /// # Returns
    ...
    pub fn intersection(
        ...
        // This is important for PHP versions that may iterate over uninitialized padding bytes
        unsafe {
    ...
    ///
    /// This method creates a PHP union type where each element is a class/interface
    /// type (e.g., `Foo|Bar`). For primitive type unions, use [`Self::union_primitive`].
    ///
    /// # Parameters
    ///
    ...
    /// # Panics
    ///
    /// Panics if fewer than 2 class names are provided (unless `allow_null` is true,
    /// in which case 1 is acceptable).
    #[must_use]
    ...
    /// Checks if this type contains a type list (union or intersection with classes).
    #[must_use]
    ...
    ///
    /// DNF types are unions where each element can be either a simple class/interface
    /// or an intersection group. For example: `(Countable&Traversable)|ArrayAccess`
    ///
    /// # Parameters
    ///
