...
impl<'a> Args<'a> {
    ...
    /// Parses `#[php(types = "...")]` or `#[php(union = "...")]` attribute from
    /// parameter attributes. Returns the parsed PHP type declaration if found.
    ///
    /// Supports:
...
fn php_type_name_to_data_type(type_name: &str) -> Option<TokenStream> {
    let tokens = match type_name.trim() {
        "int" | "long" => quote! { ::ext_php_rs::flags::DataType::Long },
        "string" => quote! { ::ext_php_rs::flags::DataType::String },
        "bool" | "boolean" => quote! { ::ext_php_rs::flags::DataType::Bool },
        "float" | "double" => quote! { ::ext_php_rs::flags::DataType::Double },
