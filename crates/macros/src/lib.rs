...
/// the field name converted to camelCase: the field `first_name` gives the
/// property `firstName`. Property types must implement `IntoZval` and
/// `FromZval`.
///
...
/// method that reads the value with `#[php(getter)]` and the method that writes
/// it with `#[php(setter)]`. The property name is the method name without its
/// `get_` or `set_` prefix, converted to camelCase. This is the same rule as
/// for `#[php(prop)]` fields: `get_first_name` gives the property `firstName`.
...
///
/// If you want a different property name, pass a `name` or `change_case` option
/// to the `#[php]` attribute: `#[php(getter, name = "first_name")]` or
...
/// getters or two setters. The getter and the setter of one property must have
/// the same visibility. Each of these cases is a compile error.
///
/// A method property cannot have the same name as a `#[php(prop)]` field of the
/// struct. The two macros are expanded separately, so this is not a compile
/// error. The extension fails to load with this error:
///
/// ```text
/// property `Book::$title` is declared twice
/// ```
///
/// The PHP type of the property comes from the return type of the getter. If
/// the property has no getter, the type comes from the argument of the setter.
/// The documentation of the property comes from the getter, or from the setter
/// when the getter has none. Like field properties, the type must implement
