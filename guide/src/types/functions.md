...
`[$object, 'method']` array) and allows you to call it from Rust.
...
For performance-critical code that calls the same PHP function repeatedly,
`CachedCallable` provides optimized function calls by caching the function
lookup. This avoids the overhead of resolving the function name on every call.
...
- **`Function`**: Performs a function lookup on each `try_from_*` call. Good for
  one-off calls.
- **`ZendCallable`**: Validates the callable on creation. Use when receiving
  callbacks from PHP.
- **`CachedCallable`**: Caches the function lookup. Best for repeated calls to
  the same function in a loop or hot path.
